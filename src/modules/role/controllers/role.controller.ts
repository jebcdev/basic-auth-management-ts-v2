...
export class RoleController {
    ...
    public async getAll(
        ...
        try {
            ...
            const data: RoleEntity[] | null =
                await this.service.getAll();

            // Si no se encontraron roles, devuelve un error 404.
            if (!data) return res.status(404).json("Roles Not Found");
    ...
    public async getById(
        ...
        try {
            ...
            if (!data) return res.status(404).json("Role Not Found");
    ...
    public async createNew(
        ...
        try {
            ...
            const dto: CreateRoleDto = plainToInstance(
                CreateRoleDto,
                req.body
            );
            ...
            if (errors.length > 0) {
                return res.status(400).json({
                    message:
                        "Validation Error | RoleController CreateNew",
            ...
            if (exists)
                return res
                    .status(400)
                    .json(`Role Already Exists: ${exists.name}`);
            ...
            if (!data) {
                return res.status(500).json("Error Creating Role");
            }
    ...
    public async updateById(
        ...
        try {
            ...
            // Llama al servicio para obtener el rol por ID.
            const toUpdate: RoleEntity | null =
            ...
            const dto: UpdateRoleDto = plainToInstance(
                UpdateRoleDto,
                req.body
            );
            ...
            if (errors.length > 0) {
                return res.status(400).json({
                    message:
                        "Validation Error | RoleController UpdateById",
                    errors: errors.map((err) => {
                        return {
                            property: err.property,
                            constraints: err.constraints,
            ...
            if (!updatedData) {
                return res.status(500).json("Error Updating Role");
            }
    ...
    public async deleteById(
        ...
        try {
            ...
            if (!deleteResult)
                return res.status(500).json("Error Deleting Role");
